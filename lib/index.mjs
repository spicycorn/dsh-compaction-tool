 ...
 */
function selectKeepBoundary(session             , keep            )                {
 ...
 */
function cutBalancesBefore(session             )                                                   {
  const nodes = session.surface.nodes;
  ...
  let running = 0;
  for (let i = 0; i < nodes.length; i += 1) {
    if ((events[nodes[i]]                       )?.seq !== nodes[i]) return { prefix: null, corruptSeq: nodes[i] };
...
function isBalancedCut(prefix                 , idx        )          {
...
function isTransientSummarizationFailure(message                    )          {
 ...
 */
function inputBudgetTokens(contextWindow                    , summaryMaxTokens        )         {
...
function hasUnknownContextWindow(contextWindow                    )          {
...
async function runCompression(
  ...
  const nodeTokens = new Map                (measurement.nodes.map((node) => [node.seq, node.tokens]));
  ...
  for (let ci = 0; ci < chunks.length; ci += 1) {
    ...
    const textBlocks = outcome.blocks.filter((block)                                           => block.type === "text");
